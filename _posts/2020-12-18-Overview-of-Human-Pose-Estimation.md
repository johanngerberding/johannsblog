...
![Some examples from the original LSP dataset]({{ '/assets/imgs/human_pose_estimation/lsp_examples.png' | relative_url}}) <center> Figure 2. Some examples from the original LSP dataset [] </center> 
...
![Some examples from the original MPII dataset]({{ '/assets/imgs/human_pose_estimation/mpii_examples.png' | relative_url}}){: style="width: 80%; margin: 0 auto; display: block;"} <center> Figure 3. Some examples from the original MPII dataset [] </center> 
...
![The model architecture of Convolutional Pose Machines]({{ '/assets/imgs/human_pose_estimation/conv_pose_machines_architecture.png' | relative_url}}){: style="width: 70%; margin: 0 auto; display: block;"} <center> Figure 7. The model architecture of [36] </center> 
...
![Convolutional Pose Machines joint detections]({{ '/assets/imgs/human_pose_estimation/conv_pose_machines_joint_detections_on_stages.png' | relative_url}}){: style="width: 70%; margin: 0 auto; display: block;"} <center> Figure 8. CPM joint detections [36] </center> 
...
![Hourglass module]({{ '/assets/imgs/human_pose_estimation/hourglass_module.png' | relative_url}}){: style="width: 70%; margin: 0 auto; display: block;"} <center> Figure 12. Hourglass module [27] </center> 
...
![PRM variants]({{ '/assets/imgs/human_pose_estimation/prm_variants.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 15. PRM variants [39] </center> 
...
![PRM results on MPII]({{ '/assets/imgs/human_pose_estimation/prm_results.png' | relative_url}}){: style="width: 65%; margin: 0 auto; display: block;"} <center> Figure 16. PRM results on MPII [39] </center> 
...
![Associative Embedding architecture]({{ '/assets/imgs/human_pose_estimation/asso_architecture.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 17. Associative Embedding architecture [26] </center> 
...
![MPII (left) and COCO (right) results]({{ '/assets/imgs/human_pose_estimation/rmpe_eval.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 22. MPII (left) and COCO (right) results [9] </center> 
...
![CPN architecture]({{ '/assets/imgs/human_pose_estimation/cpn_architecture.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 23. CPN architecture [6] </center> 
...
![CPN output heatmaps]({{ '/assets/imgs/human_pose_estimation/cpn_output.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 24. CPN output heatmaps [6] </center> 
...
![CPN results on COCO test-dev]({{ '/assets/imgs/human_pose_estimation/cpn_coco.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 24. CPN results on COCO test-dev [6] </center> 
...
![Pipeline of OpenPose]({{ '/assets/imgs/human_pose_estimation/openpose_architecture.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 25. Pipeline of OpenPose [4] </center> 

It takes as input an image of size $$w \times h$$ and produces the 2D locations of anatomical keypoints for each human in the image. In the first step, a convolutional neural network predicts a set of 2D confidence maps (heatmaps) $$\mathbf{S}$$ of body part locations (Figure 25 (b)) and a set of 2D vector fields $$\mathbf{L}$$ called part affinity fields (PAFs). These encode the degree of association between parts (Figure 25 (c)). The set $$\mathbf{S}=(\mathbf{S}_{1}, \mathbf{S}_{2}, ..., \mathbf{S}_{J})$$ has $$J$$ confidence maps., one per part, where $$\mathbf{S}_{J} \in \mathbb{R}^{w \times h}$$ , $$j \in \{1,...,J\}$$. The set $$\mathbf{L}=(\mathbf{L}_{1}, \mathbf{L}_{2}, ..., \mathbf{L}_{C})$$ has $$C$$ vector fields, one per limb, where $$\mathbf{L}_{c} \in \mathbb{R}^{w \times h \times 2}$$ , $$c \in \{1,...,C\}$$. In this context limbs are part pairs. Then a parsing step creates a set of bipartite matchings to associate the body part candidates. Finally the candidates are assembled into full body poses. Figure 26 shows the architecture of the model, which iteratively predicts affinity fields (blue) and detection confidence maps (beige). The predictions are refined over the successive stages $$t \in \{1,...,T\}$$ including intermediate supervision at each stage similar to the Convolutional Pose Machines approach.

![Architecture of OpenPose]({{ '/assets/imgs/human_pose_estimation/openpose_model.png' | relative_url}}){: style="width: 70%; margin: 0 auto; display: block;"} <center> Figure 26. Architecture of OpenPose [4] </center> 
...
$$\mathbf{S}^{T_{p}}=p^{t}(\mathbf{F},\mathbf{L}^{T_{p}}), \forall t = T_{p}$$ 

$$\mathbf{S}^{t}=p^{t}(\mathbf{F},\mathbf{L}^{T_{p}}, \mathbf{S}^{t-1}), \forall T_{p} < t \leq T_{p} + T_{c}$$
...
$$f_{\mathbf{L}}^{t_{i}} = \displaystyle\sum_{c=1}^{C} \displaystyle\sum_{\mathbf{p}} \mathbf{W}(\mathbf{p}) \cdot \| \mathbf{L}_{c}^{t_{i}}(\mathbf{p}) - \mathbf{L}_{c}^{*}(\mathbf{p}) \|_{2}^{2}$$ 

$$f_{\mathbf{S}}^{t_{k}} = \displaystyle\sum_{j=1}^{J} \displaystyle\sum_{\mathbf{p}} \mathbf{W}(\mathbf{p}) \cdot \| \mathbf{S}_{j}^{t_{k}}(\mathbf{p}) - \mathbf{S}_{j}^{*}(\mathbf{p}) \|_{2}^{2}$$
...
![MPII results of OpenPose]({{ '/assets/imgs/human_pose_estimation/openpose_mpii.png' | relative_url}}){: style="width: 70%; margin: 0 auto; display: block;"} <center> Figure 27. MPII results of OpenPose [4] </center> 
...
![Runtime of OpenPose]({{ '/assets/imgs/human_pose_estimation/openpose_runtime.png' | relative_url}}){: style="width: 85%; margin: 0 auto; display: block;"} <center> Figure 28. Runtime of OpenPose [4] </center> 
...
![Architecture of HRNet]({{ '/assets/imgs/human_pose_estimation/hrnet_architecture.png' | relative_url}}){: style="width: 70%; margin: 0 auto; display: block;"} <center> Figure 29. Architecture of HRNet [33] </center> 
...
![HRNet exchange units]({{ '/assets/imgs/human_pose_estimation/hrnet_exchange_units.png' | relative_url}}){: style="width: 60%; margin: 0 auto; display: block;"} <center> Figure 30. HRNet exchange units [33] </center> 
...
![Results on COCO test-dev]({{ '/assets/imgs/human_pose_estimation/hrnet_coco.png' | relative_url}}){: style="width: 100%; margin: 0 auto; display: block;"} <center> Figure 31. Results on COCO test-dev [33] </center> 
...
![Weight transfer scheme]({{ '/assets/imgs/human_pose_estimation/evopose_weight_transfer.png' | relative_url}}){: style="width: 70%; margin: 0 auto; display: block;"} <center> Figure 35. Weight transfer scheme [24] </center> 
...
![EvoPose2D-S architecture]({{ '/assets/imgs/human_pose_estimation/evopose_architecture.png' | relative_url}}){: style="width: 50%; margin: 0 auto; display: block;"} <center> Figure 36. EvoPose2D-S architecture [24] </center> 
...
-   Multi-context attention for human pose estimation (2017) [8]

-   SimpleBaselines (2018) [38]
